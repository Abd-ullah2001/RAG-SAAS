...
async def query_rag(
    ...
    try:
        ...
        if not context_data:
            ...
            # 2. Generate final answer using context and agent tools
            answer, tool_sources = await generate_response(request.question, context_data, user_id)
            
            # 3. Format response context (Merging document chunks + tool-based citations)
            formatted_context = [
                ContextChunk(
                    content=c["content"],
                    metadata=c["metadata"],
                    score=c.get("score", 1.0)
                ) for c in (context_data + tool_sources)
            ]
